...
public class AI extends Player {
    ...
    private ArrayList<Move> preferMoves(Game game) {
        ...
        if (dif == difficulty.Defensive) {
            moves = calcPreferMoves(game, false);
        }
        if (dif == difficulty.Aggressive) {
            moves = calcPreferMoves(game, true);
        }
        if (moves.size() == 0) {
            moves = AIMoves(game);
        }
